...
* You can avoid a `useState` re-render update loop by wrapping its updater with a `useCallback` then passing a function as it's argument to give you access to the current state [read more](https://kentcdodds.com/blog/should-i-usestate-or-usereducer#usestate-implementation-1)
* Throwing an error in a reducers default block make it easier to debug invalid action types 
